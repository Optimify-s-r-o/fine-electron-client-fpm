...
export const ResizablePanel = ({
  ...
  minSize: number;
  maxSize: number;
  afterResize?: (newSize: number, sizeOnCursor: number, e: MouseEvent) => boolean | void;
}) => {
  const borderRef = React.useRef<HTMLDivElement>();
  const styleKey = direction === 'right' || direction === 'left' ? 'width' : 'height';
  ...
  React.useEffect(() => {
    ...
    const mouseMove = (e: any) => {
      ...
      (element?.parentNode?.childNodes[0] as HTMLElement).style[styleKey] = newSize + 'px';
  ...
  return (
    <Wrapper direction={direction}>
      <Content style={{ [styleKey]: initialSize }}>{children}</Content>
...
const Border = styled.div<{ direction: any; ref: any }>`
  ...
  width: ${(props) => (props.direction === 'right' || props.direction === 'left' ? '2px' : 'auto')};
  height: ${(props) => (props.direction === 'down' || props.direction === 'up' ? '2px' : 'auto')};

  background-color: ${(props) => props.theme.colors.background.menu};

  cursor: ${(props) =>
    props.direction === 'right' || props.direction === 'left' ? 'ew-resize' : 'ns-resize'};
`;
...
const Handle = styled.div<{ direction: any }>`
  ...
  width: ${(props) =>
    props.direction === 'right' || props.direction === 'left' ? '6px' : '100px'};
  height: ${(props) =>
    props.direction === 'right' || props.direction === 'left' ? '100px' : '6px'};
