...
export const ImportJobData = ({ project }: { project?: ProjectDto | null }) => {
  ...
  const handleImport: ProgressRunFunctionWithFiles = async (
    ...
    // Do actual work
    for (let i = 0; i < files.length; i++) {
      setItemStatus(i, ProgressStatus.Running, 'Test');

      const fileRes = await createFileToWrite(files[i]);

      if (fileRes.success) {
      } else {
        setItemStatus(i, ProgressStatus.Fail, 'To je špatně');
        continue;
      }

      const res = await openFileInfoExe(
        (files[i] as any).path,
        fileRes?.file as string,
        fileRes?.directory as string
      );
      ...
      setItemStatus(
        i,
        res.exitCode === 0 ? ProgressStatus.Success : ProgressStatus.Fail,
        res.exitCode === 0 ? 'OK' : 'To je špatně'
      );
    }
