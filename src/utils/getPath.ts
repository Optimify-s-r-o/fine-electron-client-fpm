...
export function getPath<TRoot, T>(proxy: ObjProxyArg<TRoot, T> | any): PropertyKey[] {
...
export function lastPathMember<TRoot, T>(proxy: ObjProxyArg<TRoot, T> | any): ProxyProperty {
...
export function classPath<TRoot, T>(proxy: ObjProxyArg<TRoot, T> | any): ProxyProperty {
  const path = getPath(proxy);
...
export function translationPath<TRoot, T>(proxy: ObjProxyArg<TRoot, T> | any): ProxyProperty {
  const path = getPath(proxy);
